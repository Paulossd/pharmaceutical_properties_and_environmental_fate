# Data and Derived-Output License

## Scope

Unless otherwise stated, original data compilations, harmonised datasets, derived analytical outputs, metadata, and other original data products created by Paolo Tufoni and Luís M. Nunes and explicitly identified as author-generated material in this repository are made available under the **Creative Commons Attribution 4.0 International License (CC BY 4.0)**.

Copyright © 2026 Paolo Tufoni, José Paulo Monteiro and Luís Miguel Nunes.

The CC BY 4.0 license applies only to the authors' original contributions. It does not grant rights to third-party material incorporated into, referenced by, or used to construct the datasets.

## Third-party data and information

The underlying evidence synthesis incorporates information obtained from published literature and third-party databases, including, where applicable, EPI Suite, PubChem, and DrugBank. Such material remains subject to the copyright, database rights, licenses, terms of use, and other conditions applicable to its respective source.

The inclusion of a third-party value, reference, or database-derived datum in an author-generated compilation does not imply that the underlying third-party material has been relicensed under CC BY 4.0.

Source and provenance information are retained in the corresponding data tables and documentation wherever applicable. Users are responsible for determining and complying with the terms governing any third-party material before reusing or redistributing it.

## The SM_2 dataset

SM_2.xlsx is included in this repository, containing the reference table, the harmonised datasets, and the derived analytical tables. The raw extracted records (Tables SM_2.2–SM_2.3), which incorporate values derived from third-party sources subject to source-specific terms, are provided as placeholders; the complete raw workbook may be provided by the authors upon reasonable request, subject to applicable copyright, database rights, licensing terms, and redistribution conditions.

Neither inclusion of the harmonised and derived tables here, nor provision of the complete raw workbook upon request, constitutes a grant of redistribution rights for third-party material. The source and provenance of third-party-derived values are retained in the corresponding tables, and any recipient remains responsible for complying with the applicable terms of the original sources.

## Publicly available author-generated material

Author-generated code, documentation, metadata, and any data or derived outputs explicitly identified as original and released under this license may be reused under the terms of CC BY 4.0.

Under CC BY 4.0, users may:

* **Share** — copy and redistribute the material in any medium or format.
* **Adapt** — remix, transform, and build upon the material for any purpose, including commercially.

These permissions are subject to the terms of the CC BY 4.0 license.

## Attribution

For material covered by CC BY 4.0, appropriate credit must be given to Paolo Tufoni and Luís M. Nunes, a link to the CC BY 4.0 license must be provided, and any changes must be indicated.

Users must not imply that the authors endorse their use of the material.

## No additional restrictions

Users may not apply legal terms or technological measures that legally restrict others from exercising the rights granted under CC BY 4.0.

## Citation

Use of the author-generated data products or derived analytical outputs should be accompanied by citation of the associated publication and, where applicable, the corresponding archived data or software record and DOI.

Citation information is provided in the repository README and associated publication.

## Software

The R analysis code contained in this repository is **not covered by this data license**. The software is distributed separately under the **MIT License**; see `LICENSE`.

## Disclaimer

This license concerns only the authors' rights in the original material identified as being released under CC BY 4.0. It does not provide, and should not be interpreted as providing, permission to reproduce, redistribute, or otherwise use third-party material beyond the rights permitted by the applicable original source.
